...
timeout(time: 2000, unit: 'SECONDS') {
    node {
        ...
        registry_url = "https://registry-1.docker.io/v2" // Docker Hub
        ...
        docker.withRegistry("${registry_url}", "${docker_creds_id}") {
            ...
            withCredentials([usernamePassword(credentialsId: 'showerlee-github', passwordVariable: 'GIT_PASSWORD', usernameVariable: 'GIT_USERNAME')]) {
                ...
                git push origin master https://${GIT_USERNAME}:${GIT_PASSWORD}@github.com/showerlee/Jenkins-Pipeline-CI-CD-with-Helm-on-Kubernetes.git
                set -x
